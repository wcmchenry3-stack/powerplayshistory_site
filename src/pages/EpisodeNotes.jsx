...
import { EpisodeNoteListItem } from '../components/episodeNotes/EpisodeNoteListItem.jsx';
import { EmptyState } from '../components/common/EmptyState.jsx';

export default function EpisodeNotes() {
  const { t } = useTranslation('episodeNotes');
  ...
  return (
    ...
    >
      ...
      {essays.length === 0 ? (
        ...
        <section aria-label={t('pageTitle')} className="flex flex-col gap-10">
          {essays.map((note) => (
            <EpisodeNoteListItem key={note.slug} note={note} />
          ))}
        </section>
      )}
    </main>
